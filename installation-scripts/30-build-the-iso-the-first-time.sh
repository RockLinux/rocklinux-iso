...
echo
	...
	#rocklinuxVersion='v22.10.03'

	isoLabel='rocklinux-'$(date +%Y.%m.%d)'-x86_64.iso'

	# setting of the general parameters
	archisoRequiredVersion="archiso 66-1"
	buildFolder=$HOME"/rocklinux-build"
	outFolder=$HOME"/rocklinux-Out"
	archisoVersion=$(sudo pacman -Q archiso)

	echo "################################################################## "
	echo "Building the desktop                   : "$desktop
	#echo "Building version                       : "$rocklinuxVersion
...
echo "- Getting Rocklinux key and mirror"
tput sgr0
echo "################################################################## "
echo
	...
	package="rocklinux-keyring"

	#checking if application is already installed or else install
	if pacman -Qi $package &> /dev/null; then

			echo "################################################################"
			echo "Rocklinux keyring is already installed"
			echo "################################################################"

	else

		wget https://github.com/RockLinux/rocklinux_repo/raw/main/x86_64/rocklinux-keyring-2.1-6-any.pkg.tar.zst -O /tmp/rocklinux-keyring-2.1-6-any.pkg.tar.zst
		sudo pacman-key --init
		sudo pacman-key --populate
		sudo pacman -U --noconfirm --needed /tmp/rocklinux-keyring-2.1-6-any.pkg.tar.zst
		sudo pacman-key --populate
	fi
...
echo
	...
	#profiledef.sh
	oldname1='iso_name="rocklinux"'
	newname1='iso_name="rocklinux"'

	oldname2='iso_label="rocklinux"'
	newname2='iso_label="rocklinux"'
